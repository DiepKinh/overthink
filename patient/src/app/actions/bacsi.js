...
import BacSiServices from '@services/BacSiServices';
import ImageServices from '@services/ImageServices';

export const getTatCaBacSi = () => {
  return dispatch => {
    dispatch({type: ActionTypes.GET_ALL_BACSI_REQUEST});

    return BacSiServices.getTatCaBacSi()
      .then(({data}) => {
        dispatch({type: ActionTypes.GET_ALL_BACSI_SUCCESS, data});
      })
      .catch(message => {
        dispatch({type: ActionTypes.GET_ALL_BACSI_FAIL, message});
      });
  };
};

export const getBacSiTheoChuyenKhoa = params => dispatch => {
  dispatch({type: ActionTypes.GET_BACSI_CHUYENKHOA_REQUEST});
  const body = {
    ...params,
  };
  console.log('======params', body.tenchuyenkhoa);

  return BacSiServices.getBacSiTheoChuyenKhoa(body)
    .then(({data}) => {
      dispatch({type: ActionTypes.GET_BACSI_CHUYENKHOA_SUCCESS, data});
    })
    .catch(message => {
      dispatch({type: ActionTypes.GET_BACSI_CHUYENKHOA_FAIL, message});
    });
};
...
//     desc: params.desc,
//   };
//   return BacSiServices.addProduct(body)
//     .then(({data}) => {
...
//   return BacSiServices.deleteProduct(productId)
//     .then(() => {
//       dispatch({type: ActionTypes.DELETE_PRODUCT_SUCCESS, id: productId});
//     })
//     .catch(message => {
...
//   return BacSiServices.editProduct(params)
//     .then(({data}) => {
...
//             category => category.localId === categoryId,
//           );
...
//         }
//         const {data} = await BacSiServices.addProduct(params);
...
//         if (images.length > 0) {
//           const uploadedImageUris = images.filter(
...
//         }
//         params.images = images;
//         const {data} = await BacSiServices.editProduct(params);
//         return data;
//       } catch (error) {
//         return error;
//       }
...
//       try {
//         const {data} = await BacSiServices.deleteProduct(id);
//         return {...data, id};
//       } catch (error) {
//         return error;
//       }

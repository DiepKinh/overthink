...
public class ThermalPrinterModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod
  public void printBluetooth(String payload, boolean autoCut, boolean openCashbox, double mmFeedPaper,
      double printerDpi, double printerWidthMM, double printerNbrCharactersPerLine, Promise promise) {
    this.jsPromise = promise;
    ...
    if (ContextCompat.checkSelfPermission(getCurrentActivity(),
      ...
      try {
        this.printIt(BluetoothPrintersConnections.selectFirstPaired(), payload, autoCut, openCashbox, mmFeedPaper,
  ...
  private void printIt(DeviceConnection printerConnection, String payload, boolean autoCut, boolean openCashbox,
      double mmFeedPaper, double printerDpi, double printerWidthMM, double printerNbrCharactersPerLine) {
    try {
      EscPosCharsetEncoding encoding = new EscPosCharsetEncoding("GB18030", 0);
      EscPosPrinter printer = new EscPosPrinter(printerConnection, (int) printerDpi, (float) printerWidthMM,
          (int) printerNbrCharactersPerLine, (EscPosCharsetEncoding) encoding);
